...
def register(request):
  if request.method=='POST':
    ...
    if form.is_valid():
      ...
      send_email.send()
      # messages.success(request, 'Check your email to activate you account!.')
...
def login(request):
  if request.method=='POST':
    ...
    if user is not None:
      ...
      messages.error(request,'Invalid Login credentions')
...
def forgotPassword(request):
  if request.method == 'POST':
    ...
    if Account.objects.filter(email=email).exists():
      ...
      messages.success(request,'Password reset mail has been sent you email address.')
      return redirect("login")
